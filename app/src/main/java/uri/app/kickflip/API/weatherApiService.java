...
public class weatherApiService {
    ...
    public void getWeatherByCoords(double lat, double lon, WeatherCallback callback) {
        executor.execute(() -> {
            try {
                String currentUrl = BASE_URL + "?lat=" + lat + "&lon=" + lon
                        + "&appid=" + API_KEY + "&units=metric";
                String forecastUrl = FORECAST_URL + "?lat=" + lat + "&lon=" + lon
                        + "&appid=" + API_KEY + "&units=metric";
                fetchAndBuildSpot(currentUrl, forecastUrl, callback);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void getWeather(String location, WeatherCallback callback) {
        executor.execute(() -> {
            try {
                String currentUrl  = BASE_URL      + "?q=" + location + "&appid=" + API_KEY + "&units=metric";
                String forecastUrl = FORECAST_URL  + "?q=" + location + "&appid=" + API_KEY + "&units=metric";
                fetchAndBuildSpot(currentUrl, forecastUrl, callback);
            } catch (Exception e) {
                e.printStackTrace();
                mainHandler.post(() -> callback.onError(e.getMessage()));
    ...
    private void fetchAndBuildSpot(String currentUrl, String forecastUrl,
        ...
        if (isRaining) {
            ...
            if (totalRainMm == 0) totalRainMm = 1.0;
        } else if (rain1h > 0) {
        ...
        spot.calculateGroundStatus(isRaining, lastRainTime, totalRainMm,
                temp, humidity, windSpeedMs, cloudCoverPct, isDaytime);
